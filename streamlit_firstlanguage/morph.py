...
def get_morph_with_txt(text, apiKey, lang="en"):
    # check if apikey has value
    if apiKey is None:
        raise Exception("No API Key provided")
    # check if text has value
    if text is None:
        raise Exception("No text to calculate morph features provided")
...
def get_morph_with_url(url, apiKey,contentType, lang="en"):
    # check if apikey has value
    if apiKey is None:
        raise Exception("No API Key provided")
    # check if text has value
    if url is None:
        raise Exception("No text to calculate morph features")
...
def morph(name, apiKey, morphText=True, title=None, txtAreaLabel=None,
    ...
    if morphText:
        with st.container():
            ...
            txt = st.text_area(txtAreaLabel, height=height,value="",
                               max_chars=1000000, help=helpText, key="morph_text_area")
            label="Enter language ISO code:"
            if langLabel is not None:
                label = langLabel
            if showLang:
                langSelected = st.text_input(label, value="en",
                               max_chars=2, key="morph_lang1")

            if st.button('Submit',key="morph_submit"):
                if txt != "":
                    result = get_morph_with_txt(
        ...
        with st.container():
            ...
            if showLang:
                langSelected = st.text_input(label, value="en",
                               max_chars=2, key="morph_lang2")

            contentType= st.selectbox("Select content type", ("Select Content Type", "html", "plaintext", "pdf", "docx"), key="morph_select_content_type")

            if st.button('Submit',key="morph_submit_1"):
                if (txt != "" and contentType != "Select Content Type"):
                    result = get_morph_with_url(
